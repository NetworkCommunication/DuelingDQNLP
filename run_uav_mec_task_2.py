"""
这一版本是成功的，对于Dueling DQN来解决时间优化问题
"""
...
with tf.variable_scope('DoubleDueling_DQN'):
    double_dueling_DQN = DoubleDQN(
        n_actions=env.n_actions, n_features=env.n_features, memory_size=MEMORY_SIZE,
        e_greedy_increment=0.001, dueling=3, sess=sess, output_graph=True)
...
# 优化的是无人机的轨迹以及任务的卸载
UAV_trajectory_tsp = np.zeros((Episodes, env.N_slot, 3), dtype=np.float32)  # 无人机的轨迹
Task_offloading_tsp = np.zeros((Episodes, env.N_slot, env.TKs), dtype=np.float32)  # 任务卸载
UAV_flight_time_tsp = np.ones((Episodes, env.N_slot), dtype=np.float32)  # 时间段，修改为1
UE_Schedulings_tsp = np.zeros((Episodes, env.N_slot, env.GTs), dtype=np.float32)  # 用户调度
...
UAV_trajectory_double_dueling = np.zeros((Episodes, env.N_slot, 3), dtype=np.float32)
Task_offloading_double_dueling = np.ones((Episodes, env.N_slot, env.TKs), dtype=np.float32)
UAV_flight_time_double_dueling = np.ones((Episodes, env.N_slot), dtype=np.float32)
UE_Schedulings_double_dueling = np.zeros((Episodes, env.N_slot), dtype=np.float32)  # 用户调度
Task_offloading_time_double_dueling = np.ones((Episodes, env.N_slot, env.TKs), dtype=np.float32)  # 任务时间分配
Convergence_double_dueling = np.ones((Episodes, env.N_slot), dtype=np.float32)
Slot_double_dueling = np.zeros((1, Episodes), dtype=np.int)
...
def train(RL):
    ...
    for ep in range(Episodes):
        ...
        while (env.finish == False):
            ...
            if (RL.dueling == 3):
                UAV_trajectory_double_dueling[ep, slot, :] = observation_[:]
                UE_Schedulings_double_dueling[ep, slot] = index_wk
                Task_offloading_double_dueling[ep, slot, :] = action[-env.TKs - 1:-1]
                UAV_flight_time_double_dueling[ep, slot] = action[-1] / 10
                Task_offloading_time_double_dueling[ep, slot, :] = t_n_t_c
                Convergence_double_dueling[ep, slot] = reward
                Slot_double_dueling[0, ep] = Slot_double_dueling[0, ep] + 1
        ...
        if (RL.dueling == 3):
            UAV_trajectory_double_dueling[ep, :] = env.UAV_FLY(UAV_trajectory_double_dueling[ep, :],
                                                               Slot_double_dueling[0, ep])
        # for TSP
        UAV_trajectory_tsp[ep, :] = env.UAV_trajectory_tsp[:]
        Task_offloading_tsp[ep, :] = np.ones((env.N_slot, env.TKs), dtype=np.float32)
        # UAV_flight_time_tsp[ep, :] = 3 * np.ones((1, env.N_slot), dtype=np.float32)
        UAV_flight_time_tsp[ep, :] = np.ones((1, env.N_slot), dtype=np.float32)
        scheduling = np.zeros((env.N_slot, env.GTs), dtype=np.float32)
        for i in range(env.N_slot):
            scheduling[i, 0] = 1
        UE_Schedulings_tsp[ep, :] = scheduling
    return RL.q


# 分别训练了两次，所以tsp路径出了两个图
q_natural = train(natural_DQN)
# q_double = train(double_DQN)
# q_dueling = train(dueling_DQN)
# q_double_dueling = train(double_dueling_DQN)
# Double DQN
...
# Dueling DQN
np.save(file="UAV_trajectory_double_dueling.npy", arr=UAV_trajectory_double_dueling)
np.save(file="UE_Schedulings_double_dueling.npy", arr=UE_Schedulings_double_dueling)
np.save(file="Task_offloading_double_dueling.npy", arr=Task_offloading_double_dueling)
np.save(file="UAV_flight_time_double_dueling.npy", arr=UAV_flight_time_double_dueling)
np.save(file="Task_offloading_time_double_dueling.npy", arr=Task_offloading_time_double_dueling)
np.save(file="Convergence_double_dueling.npy", arr=Convergence_double_dueling)
np.save(file="Slot_double_dueling.npy", arr=Slot_double_dueling)
...
EPS = env.eps - 1
res.plot_UAV_GT(env.w_k, UAV_trajectory_natural, UAV_trajectory_double, UAV_trajectory_dueling,
...
res.plot_propulsion_energy(UAV_trajectory_natural, UAV_trajectory_double, UAV_trajectory_dueling,
...
res.plot_data_throughput(UAV_trajectory_natural, UAV_trajectory_double, UAV_trajectory_dueling,
                         ...
                         Slot_double_dueling)
res.plot_energy_efficiency(UAV_trajectory_natural, UAV_trajectory_double, UAV_trajectory_dueling,
                           UAV_trajectory_double_dueling,
                           UAV_flight_time_natural,
                           UAV_flight_time_double, UAV_flight_time_dueling, UAV_flight_time_double_dueling,
                           Task_offloading_natural,
                           ...
                           Task_offloading_time_double_dueling, EPS, Slot_natural, Slot_double, Slot_dueling,
                           Slot_double_dueling)
# res.plot_convergence(Convergence_natural, Convergence_dqn, EPS)
